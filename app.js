...
function sortByMes(arr){ return [...arr].sort((a,b)=>(MES_ORDER[a.toLowerCase()]||99)-(MES_ORDER[b.toLowerCase()]||99)); }
function buildCsvUrl(raw){
  const m=raw.match(/\/d\/([a-zA-Z0-9-_]+)/);
  return m ? `https://docs.google.com/spreadsheets/d/${m[1]}/gviz/tq?tqx=out:csv&sheet=REGISTRO` : raw;
}
...
function StatusBar({status,count,errorMsg}){
  const dc=status==='loading'?'loading':status==='error'?'error':'';
  return (
    <div className="status-bar">
      <div className={`status-dot ${dc}`}/>
...
function ConfigBanner({onLoad}){
  ...
  return (
    <div className="config-banner">
      ...
      <p>Cole o link da sua planilha no Google Drive.<br/>
      ...
      <div className="config-input-row">
        <input type="text" placeholder="https://docs.google.com/spreadsheets/d/SEU_ID_AQUI/..."
          value={url} onChange={e=>setUrl(e.target.value)}
          onKeyDown={e=>e.key==='Enter'&&url.trim()&&onLoad(url.trim())}/>
        <button className="btn-primary" onClick={()=>url.trim()&&onLoad(url.trim())}>Carregar</button>
      </div>
      <div className="url-hint">💡 Cole o link normal — o sistema converte automaticamente para CSV</div>
    </div>
  );
}
...
function Filters({options,filters,onChange,onClear}){
  const has=Object.values(filters).some(v=>v);
  return (
    <div className="filters-wrapper">
      ...
      <div className="filters-grid">
        ...
        {has&&<button className="btn-ghost" onClick={onClear}>✕ Limpar filtros</button>}
      </div>
    </div>
  );
}
                ...
                <tr>
                  {COLS.map(c=>(
                    <th key={c.key} className={sort.col===c.key?'sorted':''} onClick={()=>onSort(c.key)}>
                      {c.label} <span className="sort-arrow">{sort.col===c.key?(sort.dir==='asc'?'↑':'↓'):'↕'}</span>
                    </th>
                  ))}
                </tr>
              </thead>
              <tbody>
                {pageData.map((row,i)=>(
                  <tr key={i}>
                    ...
                    <td>{row['SERVIÇO']}</td>
                    <td><span className={`badge ${row['PAGAMENTO']==='Extra'?'badge-extra':'badge-diaria'}`}>{row['PAGAMENTO']}</span></td>
...
function App(){
  ...
  function handleLoad(rawUrl){
    ...
    Papa.parse(buildCsvUrl(rawUrl),{
      ...
      complete(r){
        if(r.data?.length){setAllData(r.data);setStatus('loaded');setFilters({militar:'',mes:'',servico:'',pagamento:''});}
  ...
  const chartData=useMemo(()=>{
    const c={};
    filtered.forEach(r=>{const m=r['MÊS']||'Sem mês';c[m]=(c[m]||0)+1;});
    return sortByMes(Object.keys(c)).map(mes=>({mes:capitalize(mes),total:c[mes]}));
  },[filtered]);
